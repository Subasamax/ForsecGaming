...
const GameCard = ({ gameID, title, userName }) => {
  const gameLink = `http://localhost:3000/games/${gameID}/play`;
  const imageURL = `http://localhost:3001/${gameID}/static/photo`;

  // no repeat, CSS which prevents the image from repeating, tiles horizontaly and vertically, ensure it only displays once
